...
export default function BlurryLettersWrapper({ children }: BlurryLettersWrapperProps) {
    ...
    return (
        <div ref={containerRef} className="w-full relative overflow-hidden">
            ...
            <motion.div style={{ y: y1 }} className="flex absolute blur-[10px] z-[-3] -translate-y-24 inset-0 flex-wrap gap-2 justify-around align-middle text-white/50 text-8xl">
                {CHARS.split('').map((char: string, i: number) => <span className="" key={i}>{char}</span>)}
            </motion.div>

            <motion.div style={{ y: y2 }} className="absolute blur-[10px] z-[-3] left-0 -translate-y-24 flex flex-wrap gap-4 justify-around align-middle text-white/50 text-6xl">
                {CHARS.split('').map((char: string, i: number) => <span className="" key={i}>{char}</span>)}
            </motion.div>
